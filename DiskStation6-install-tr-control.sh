...
ARG1="$1"
ROOT_FOLDER=""
SCRIPT_NAME="install-tr-control.sh"
...
PACK_NAME="master"
WEB_HOST="https://codeload.github.com/ronggang/transmission-web-control/tar.gz/"
...
MSG_MAIN_MENU="
	...
	4. Re-download the original installation script (ronggang/transmission-web-control/$SCRIPT_NAME).
...
MSG_INSTALL_SCRIPT_DOWNLOAD_COMPLETE="The download is complete. Please re-run the installation script.($SCRIPT_NAME)"
...
initValues() {
	...
	if [ "$VERSION" != "" ]; then
		...
		DOWNLOAD_URL="https://codeload.github.com/ronggang/transmission-web-control/tar.gz/$PACK_NAME"
	fi	
...
downloadInstallScript() {
	...
	curl -o "$SCRIPT_NAME" -J "https://github.com/ronggang/transmission-web-control/raw/master/release/$SCRIPT_NAME" --insecure
	# 判断是否下载成功
	if [ $? -eq 0 ]; then
