...
ARG1="$1"
ROOT_FOLDER=""
SCRIPT_NAME="install-tr-control-cn.sh"
...
WEB_HOST="https://archive.fastgit.org/ronggang/transmission-web-control/archive/"
...
MSG_END="结束"
MSG_MAIN_MENU="
	欢迎使用 Transmission Web Control 中文安装脚本(CDN版)。
	...
	2. 安装指定版本，可用于降级；
	3. 恢复到官方UI；
	4. 重新下载原作者安装脚本（ronggang/transmission-web-control/$SCRIPT_NAME）；
...
MSG_INSTALL_SCRIPT_DOWNLOAD_COMPLETE="下载完成，请重新运行安装脚本。($SCRIPT_NAME)"
...
initValues() {
	...
	if [ "$VERSION" != "" ]; then
		...
		DOWNLOAD_URL="https://archive.fastgit.org/ronggang/transmission-web-control/archive/$PACK_NAME"
	fi	
...
download() {
	...
	curl  -o "$PACK_NAME" -J "$DOWNLOAD_URL" --insecure
	# 判断是否下载成功
	if [ $? -eq 0 ]; then
...
downloadInstallScript() {
	...
	curl -o "$SCRIPT_NAME" -J "https://raw.fastgit.org/ronggang/transmission-web-control/master/release/$SCRIPT_NAME" --insecure 
	# 判断是否下载成功
	if [ $? -eq 0 ]; then
